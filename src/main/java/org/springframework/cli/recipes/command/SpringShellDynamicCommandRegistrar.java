	...
	public void registerSpringCliCommands(CommandCatalog commandCatalog,
		...
		for (Entry<Command, List<Command>> stringListEntry : commandSubcommandMap.entrySet()) {
			...
			for (Command subCommand : subCommandList) {
				...
				SpringShellGeneratorCommand springShellGeneratorCommand
						= new SpringShellGeneratorCommand(commandName, subCommandName, generatorResolver);

				Builder builder = CommandRegistration.builder()
						.command(commandName + " " + subCommandName)
						//TOOD .description when spring-shell adds it
						.help(subCommand.getHelp())
						.withTarget()
							.method(springShellGeneratorCommand, "runGenerator")
						.and();
				...
				logger.debug("Adding command/subcommand " + commandName + "/" + subCommandName);
