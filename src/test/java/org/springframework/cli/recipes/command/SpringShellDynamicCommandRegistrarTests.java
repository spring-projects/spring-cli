...
public class SpringShellDynamicCommandRegistrarTests {
	...
	void testRegistrar() {
		...
		Command k8sSimple = new Command("k8s-simple", null, "simple help", null);
		Command k8sSimpleNew = new Command("new", "subcommand description", "subcommand help", null);
		...
		Command k8sSimpleNewServices = new Command("new-services",
				"Create the Kubernetes resource files for the application's services.", "new services help", null);
		...
		assertThat(commandCatalog.getRegistrations().get("k8s-simple new")).satisfies(registration -> {
			assertThat(registration.getHelp()).isEqualTo("subcommand help");
